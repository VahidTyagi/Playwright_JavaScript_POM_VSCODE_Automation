...
class HomePage extends BasePage {
  constructor(page) {
    ...
    // ===== HEADER =====
    this.logo = page.getByTestId('header-desktop-brand-link');
    ...
    // ===== HERO SECTION =====
    this.mainBanner = page.locator('[data-testid^="carousel-slide"]').first();
    ...
    // ===== OTHER SECTIONS =====
    this.authorizedSellersSection = page.locator(
      "div.w-full.md\\:py-8.lg\\:py-\\[76px\\].px-5"
    );

    this.testimonialsSection = page.locator("section").nth(6);
    this.faqSection = page.locator(".faq-widget");
  ...
  async navigate() {
    await super.navigate('https://store.wellversed.in/');
  ...
  async verifyTitle() {
    await expect(this.page).toHaveTitle(/Wellversed/);
  }
